import { test, expect } from '@playwright/test';

test.describe('Kansler.uz - Full Test Suite', () => {
  test('Submit an empty comment on a course', async ({ page }) => {
    // Given the user is logged in with phone "998111111" and password "Azizbek5600@"
    await page.goto('/');
    await page.waitForLoadState('domcontentloaded');

    const loginInput = page.getByTestId('login-phone-input')
      .or(page.getByTestId('login-email-input'))
      .or(page.getByPlaceholder(/phone|telefon|login|email/i))
      .or(page.locator('input[type="tel"]'))
      .first();
    await expect(loginInput).toBeVisible({ timeout: 15000 });
    await loginInput.fill('998111111');

    const passwordInput = page.getByTestId('login-password-input')
      .or(page.getByPlaceholder(/password|parol/i))
      .or(page.locator('input[type="password"]'))
      .first();
    await expect(passwordInput).toBeVisible({ timeout: 10000 });
    await passwordInput.fill('Azizbek5600@');

    const submitBtn = page.getByRole('button', { name: /kirish|login|sign in|enter/i })
      .or(page.getByTestId('login-submit-button'))
      .or(page.locator('button[type="submit"]'))
      .first();
    await expect(submitBtn).toBeVisible({ timeout: 10000 });
    await submitBtn.click();
    await page.waitForLoadState('domcontentloaded');

    // And the user is on "https://kansler-roan.vercel.app/courses/1"
    await page.goto('https://kansler-roan.vercel.app/courses/1');
    await page.waitForLoadState('domcontentloaded');

    // When the user leaves "[data-testid='comment-input']" empty and clicks "[data-testid='comment-submit-btn']"
    const commentInput = page.getByTestId('comment-input');
    await expect(commentInput).toBeVisible({ timeout: 10000 });
    await commentInput.fill('');

    const commentSubmitBtn = page.getByTestId('comment-submit-btn');
    await expect(commentSubmitBtn).toBeVisible({ timeout: 10000 });
    await commentSubmitBtn.click();
    await page.waitForLoadState('domcontentloaded');

    // Then "[data-testid='comment-error']" validation error is visible
    const commentError = page.getByTestId('comment-error');
    await expect(commentError).toBeVisible({ timeout: 10000 });
  });
});