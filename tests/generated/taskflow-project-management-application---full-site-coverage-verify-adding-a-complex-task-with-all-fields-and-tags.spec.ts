...
test.describe('TaskFlow Project Management Application - Full Site Coverage', () => {
  test('Verify adding a complex task with all fields and tags', async ({ page }) => {
    ...
    await page.getByTestId('task-description-textarea').click();
    ...
    await page.getByTestId('task-start-date-input').click();
    ...
    await page.getByTestId('task-due-date-input').click();
    ...
    await page.getByTestId('tag-text-input').click();
    ...
    await expect(page.getByTestId('toast-message')).toBeVisible({ timeout: 15000 });
